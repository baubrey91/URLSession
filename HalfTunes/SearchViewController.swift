...
//  HalfTunes
//
//  Created by Brandon Aubrey on 02/6/17.
//
...
class SearchViewController: UIViewController {
    ...
    func trackIndexForDownload(downloadTask: URLSessionDownloadTask) -> Int? {
...
extension SearchViewController : URLSessionDownloadDelegate {
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        if let url = downloadTask.originalRequest?.url?.absoluteString {
            ...
            if let trackIndex = trackIndexForDownload(downloadTask: downloadTask) {
                DispatchQueue.main.async {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        if let downloadUrl = downloadTask.originalRequest?.url?.absoluteString, let download = activeDownloads[downloadUrl] {
            ...
            if let trackIndex = trackIndexForDownload(downloadTask: downloadTask), let trackCell = tableView.cellForRow(at:(IndexPath(row: trackIndex, section: 0))) as? TrackCell {
                DispatchQueue.main.async {
...
extension SearchViewController: URLSessionDelegate {
    func urlSessionDidFinishEvents(forBackgroundURLSession session: URLSession) {
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
            if let completeionHandler = appDelegate.backgroundSessionCompleteionHandler {
                appDelegate.backgroundSessionCompleteionHandler = nil
                DispatchQueue.main.async {
                    completeionHandler()
                }
            }
